...
- `scripts/worker.ts` — the background worker (`npm run worker`, or `Dockerfile.worker` to containerize it) that renders uploaded plan PDFs to pages and reads them with AI to produce priced material/labor findings. See [`docs/architecture/ai-plan-reading-pipeline.md`](docs/architecture/ai-plan-reading-pipeline.md).
