...
function fixture(options: { user?: string | null; owner?: boolean; rpcError?: string; issued?: Record<string, unknown> } = {}) {
  ...
  const db = {
    ...
    from: (table: string) => ({ select() { return this; }, eq() { return this; }, maybeSingle: async () => ({ data: table === 'pilot_cohorts' ? { capacity: 35, budget_cents: 10000, reserved_cents: 0, enabled: true } : { is_platform_admin: options.owner ?? true }, error: null }) }),
...
test('owner dashboard sanitizes hashes and exposes server-derived budget data', async () => {
  ...
  const result = await response.json(); assert.equal(result.cohortBudgetCents, 10000); assert.equal(result.capacity, 35); assert.equal(result.invitations[0].reserved_cents, 75); assert.equal(result.invitations[0].enrollment_expires_at, '2099-02-01T00:00:00Z'); assert.equal('token_hash' in result.invitations[0], false);
});
