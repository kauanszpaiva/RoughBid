...
test('POST accepts empty optional category and assembly description fields used by the current UI', async () => {
  let called: Record<string, unknown> | undefined;
  const client = {
    auth: { getUser: async () => ({ data: { user: { id: 'user-1' } }, error: null }) },
    rpc: async (_name: string, args: Record<string, unknown>) => { called = args; return { data: [row], error: null }; },
  };
  const response = await handleWorkspaceCatalogRequest(new Request(
    'https://roughbid.test/api/workspaces/ws-1/estimating-catalog',
    { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
      materials: [{ ...material, category: '' }],
      assemblies: [{ ...assembly, category: '', description: '' }],
      expectedRevision: 0,
    }) },
  ), client as never);
  assert.equal(response.status, 201);
  assert.equal((called?.p_materials as Array<Record<string, unknown>>)[0]?.category, '');
  assert.equal((called?.p_assemblies as Array<Record<string, unknown>>)[0]?.description, '');
});
...
test('POST rejects material rows without a cost before calling the database', async () => {
  let called = false;
  const client = {
    auth: { getUser: async () => ({ data: { user: { id: 'user-1' } }, error: null }) },
    rpc: async () => { called = true; return { data: [], error: null }; },
  };
  const { unitCost: _unitCost, ...withoutCost } = material;
  const response = await handleWorkspaceCatalogRequest(new Request(
    'https://roughbid.test/api/workspaces/ws-1/estimating-catalog',
    { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ materials: [withoutCost], assemblies: [], expectedRevision: 0 }) },
  ), client as never);
  assert.equal(response.status, 400);
  assert.equal(called, false);
});
