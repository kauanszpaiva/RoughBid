...
function optionalText(value: unknown, label: string, max: number): string | undefined {
  ...
  if (typeof value !== 'string' || value.trim().length > max) {
    throw new CatalogApiError(400, `${label} must be at most ${max} characters`);
  }
  return value.trim();
}
...
function validateMaterials(value: unknown): Record<string, unknown>[] {
  ...
  return value.map((candidate, index) => {
    ...
    return {
      ...
      category: optionalText(item.category, `materials[${index}].category`, 120) ?? '',
...
function validateAssemblies(value: unknown): Record<string, unknown>[] {
  ...
  return value.map((candidate, index) => {
    ...
    return {
      ...
      category: optionalText(item.category, `assemblies[${index}].category`, 120) ?? '',
      description: optionalText(item.description, `assemblies[${index}].description`, 1_000) ?? '',
