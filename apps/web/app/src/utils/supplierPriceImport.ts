...
const units=new Set<UnitType>(['SF','LF','EA','CY','SY','HR','LS']);
...
function rows(csv:string):string[][]{
  ...
  for(let i=0;i<csv.length;i++){
    const char=csv[i]!;
    if(quoted){if(char==='"'&&csv[i+1]==='"'){field+='"';i++;}else if(char==='"')quoted=false;else field+=char;continue;}
    if(char==='"'){if(field)throw new Error('CSV contains an invalid quote.');quoted=true;}
    else if(char===','){row.push(field);field='';}
    else if(char==='\n'){row.push(field);output.push(row);row=[];field='';if(output.length>2001)throw new Error('Supplier CSV is limited to 2,000 rows.');}
...
export function parseSupplierPriceCsv(csv:string,idFactory:()=>string=()=>crypto.randomUUID()):MaterialItem[]{
  if(!csv.trim())throw new Error('Choose a non-empty CSV file.');
  const parsed=rows(csv.replace(/^\uFEFF/,''));
  const headers=(parsed.shift()??[]).map(key);
  const position=(...names:string[])=>headers.findIndex(header=>names.includes(header));
  ...
  const imported:MaterialItem[]=[];const seen=new Set<string>();
  parsed.forEach((values,index)=>{
    const line=index+2;const name=(values[nameIndex]??'').trim();
    const unit=(values[unitIndex]??'').trim().toUpperCase() as UnitType;
