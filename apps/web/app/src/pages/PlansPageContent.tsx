...
export const PlansPage: React.FC<PlansPageProps> = ({
  ...
  const handlePay = async () => {
    if (!canWrite || isPaying || isStartingAi || isUploading) return;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!canWrite || isUploading || isStartingAi || isPaying) return;
  ...
  const handleStartFreeReading = async () => {
    if (!canWrite || isStartingAi || isUploading || isPaying) return;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-[#e5e7eb] rounded-xl p-5 shadow-xs space-y-2">
            ...
            {/* Upload New Revision */}
            <label className={`w-full flex items-center justify-between px-3 py-2 bg-[#f9fafb] hover:bg-[#f3f4f6] border border-[#e5e7eb] rounded-lg text-xs font-medium text-[#111827] transition cursor-pointer ${isUploading || isStartingAi || isPaying ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}>
            ...
            {entitlementReady && freeReadingAvailable && (
              <button
                ...
                disabled={!canWrite || !selectedTrades.length || !currentRevision?.remoteFileId || currentRevision.processingStatus !== "ready" || isStartingAi || isUploading || isPaying}
            ...
            {entitlementReady && !freeReadingAvailable && !pilotActive && <button
              ...
              disabled={!canWrite || !selectedTrades.length || !aiReadingAvailable || !currentRevision?.remoteFileId || currentRevision.processingStatus !== "ready" || isStartingAi || isUploading || isPaying}
