...
import { createBillingCheckout, getMarketplaceCatalog, importSupplierPrices, type MarketplaceCatalog } from '../services/api';
import { StorageService, type StorageScope } from '../utils/storage';
import { formatCurrency } from '../utils/calculations';
import { SUPPLIER_CSV_TEMPLATE, SUPPLIER_CSV_MAX_BYTES } from '../utils/supplierPriceImport';
...
export const PriceListsPage: React.FC<{ scope: StorageScope; canWrite?:boolean; onOpenMaterials: () => void }> = ({scope,canWrite=false,onOpenMaterials}) => {
  ...
  const activeImport=React.useRef(true);
  React.useEffect(()=>{activeImport.current=true;return()=>{activeImport.current=false;};},[]);
  ...
  const importCsv=async(event:React.ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];event.target.value='';if(!file||!supplier?.entitled||!canWrite)return;
    if(file.size>SUPPLIER_CSV_MAX_BYTES){setError('Supplier CSV is limited to 2 MB.');return;}setBusy(true);setError('');setNotice('');
    try{const {materials:imported}=await importSupplierPrices(scope.workspaceId,await file.text());
      if(!activeImport.current)return;
      const identities=new Set(imported.map(item=>`${item.name.toLowerCase()}\n${(item.supplier??'').toLowerCase()}\n${item.unit}`));
      const updated=[...imported,...StorageService.getMaterials(scope).filter(item=>!identities.has(`${item.name.toLowerCase()}\n${(item.supplier??'').toLowerCase()}\n${item.unit}`))];
  ...
  return <div className="p-4 sm:p-8 max-w-6xl mx-auto space-y-6">
    ...
    </section><p className="text-xs text-slate-500">Supplier CSVs are validated by RoughBid, then your price book is saved in this browser for your account and workspace. Export a CSV backup before changing devices. Always confirm supplier quotes, effective dates, taxes, delivery, and local requirements.</p>
  </div>;
};
