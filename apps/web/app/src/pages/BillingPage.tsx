...
import { bootstrapAuth, getCapabilities, createBillingCheckout, type BillingPriceKey } from '../services/api';
...
export const BillingPage: React.FC = () => {
  ...
  return <div className="max-w-5xl mx-auto p-5 sm:p-8 space-y-6">
    ...
    <section className="grid sm:grid-cols-3 gap-4">{(['starter','pro','team'] as const).map(tier => <article key={tier} className="bg-white border rounded-xl p-5 space-y-3">
      ...
      <button disabled={platformAdmin || busy || !billingAvailable} onClick={() => choose(`plan_${tier}`)} className="rounded-lg border px-4 py-2 disabled:opacity-50">{platformAdmin ? 'Included' : billingAvailable ? 'View membership' : 'Currently unavailable'}</button>
    </article>)}</section>
