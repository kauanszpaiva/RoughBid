...
export const BillingPage: React.FC = () => {
  ...
  if (accessLoading) return <div className="max-w-5xl mx-auto p-5 sm:p-8"><h1 className="font-display text-2xl font-semibold tracking-tight text-slate-900">Billing & membership</h1><p role="status" className="text-slate-600 mt-4">Checking your account access…</p></div>;
  if (accessError) return <div className="max-w-5xl mx-auto p-5 sm:p-8 space-y-4"><h1 className="font-display text-2xl font-semibold tracking-tight text-slate-900">Billing & membership</h1><p role="alert" className="text-amber-800">{accessError}</p><button className="rounded-lg border px-4 py-2" onClick={() => setAccessRetry(value => value + 1)}>Check access again</button></div>;
