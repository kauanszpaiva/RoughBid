...
export const BillingPage: React.FC = () => {
  ...
  return <div className="max-w-5xl mx-auto p-5 sm:p-8 space-y-6">
    <header><h1 className="text-2xl font-bold">Billing & membership</h1><p className="text-slate-600 mt-2">Pay for each project before AI starts. An active company membership lowers the project price.</p></header>
    ...
    <section className="grid sm:grid-cols-3 gap-4">{(['starter','pro','team'] as const).map(tier => <article key={tier} className="bg-white border rounded-xl p-5 space-y-3">
      ...
      <button disabled={busy} onClick={() => choose(`plan_${tier}`)} className="rounded-lg border px-4 py-2 disabled:opacity-50">View membership</button>
    </article>)}</section>
