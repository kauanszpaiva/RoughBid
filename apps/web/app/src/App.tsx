...
export default function App() {
  ...
  const remoteToProject = (remote: RemoteProject, profile: UserProfile): Project => {
    ...
    const project: Project = {
      ...
      clientName: remote.client_name ?? stored?.clientName ?? "Client",
      address: stored?.address ?? remote.address_text ?? "",
      projectType: remote.project_type ?? stored?.projectType ?? "Construction Project",
      ...((remote.jurisdiction_state ?? stored?.jurisdictionState) ? { jurisdictionState: (remote.jurisdiction_state ?? stored?.jurisdictionState)! } : {}),
      ...((remote.municipality ?? stored?.municipality) ? { municipality: (remote.municipality ?? stored?.municipality)! } : {}),
      ...((remote.postal_code ?? stored?.postalCode) ? { postalCode: (remote.postal_code ?? stored?.postalCode)! } : {}),
      ...((remote.permit_date ?? stored?.permitDate) ? { permitDate: (remote.permit_date ?? stored?.permitDate)! } : {}),
  ...
  const projectPayload = (project: Project) => ({
    ...
    jurisdictionState: project.jurisdictionState,
