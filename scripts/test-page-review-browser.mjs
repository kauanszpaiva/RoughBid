...
try {
  ...
  await writeFile(path.join(root,'main.tsx'),`import React from 'react';import {createRoot} from 'react-dom/client';import {PageReviewPanel} from '../apps/web/app/src/features/plans/inspector/PageReviewPanel';createRoot(document.getElementById('root')).render(<PageReviewPanel workspaceId="w" projectId="p" fileId="f" scope="Test" canWrite={true} onBusyChange={()=>{}} onOpenJob={(id)=>window.pageHarness.reviews.push(id)}/>);`);
  const config={configFile:false,root,plugins:[react(),{name:'page-review-test-api',enforce:'pre',resolveId(source,importer){if(importer?.endsWith('/features/plans/inspector/PageReviewPanel.tsx')&&source==='../../../services/api')return '\0page-api';},load(id){if(id==='\0page-api')return stub;}}],build:{outDir:path.join(root,'dist'),emptyOutDir:true},preview:{host:'127.0.0.1',port:4183,strictPort:true}};
