...
var MAX_PLAN_BYTES = 100 * 1024 * 1024;
...
async function fetchPrivatePdf(url, headers = {}, fetcher = fetch, maxBytes = MAX_FREE_AI_PDF_BYTES) {
  ...
  try {
    for (; ; ) {
      ...
      if (length > maxBytes) {
        await reader.cancel();
        throw new ProjectApiError(413, `PDF exceeds the ${maxMb} MB free AI request limit.`);
      }
      chunks.push(value);
    }
  } finally {
