-- Expand the existing company AI spend breaker to the two explicitly approved
-- low-cost vision providers. No secret or provider endpoint is stored in Postgres.
-- Provider activation remains controlled by server-only environment flags.
...
begin
  if p_provider not in ('gemini','deepseek','kimi') or btrim(coalesce(p_model,'')) = '' then
...
comment on function public.reserve_provider_spend(uuid,uuid,uuid,uuid,text,text)
  is 'Service-only company AI spend breaker. Approved providers: Gemini, DeepSeek, Kimi.';
