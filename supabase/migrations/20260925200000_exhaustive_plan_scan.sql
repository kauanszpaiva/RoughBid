...
-- Keeps long platform-admin scans durable and raises the old 200-finding
-- persistence boundary so dense whole-plan evidence is not silently rejected.
...
begin
  ...
  set status='processing',worker_id=trim(p_worker_id),worker_lease_id=lease,
      worker_lease_expires_at=now()+interval '15 seconds',worker_heartbeat_at=now(),
      worker_attempt=worker_attempt+1,started_at=coalesce(started_at,now()),processing_error=null
  where id=j.id returning * into j;

  return jsonb_build_object(
    'skip',false,'lease_id',lease,'entitlement',ent,'quote_id',qid,
    'requested_by',j.requested_by,'workspace_id',j.workspace_id,'project_id',j.project_id,
    'file_id',j.file_id,'model',j.model,'mode',j.mode,'storage_path',f.storage_path,
    'original_name',f.original_name,'checkpoint',j.processing_checkpoint,
...
begin
  ...
  if ent='paid' then
    ...
    where id=qid and job_id=j.id and status='processing' and paid_at is not null for update;
...
create or replace function public.finish_project_reading(p_quote_id uuid,p_job_id uuid,p_summary jsonb,p_findings jsonb,p_error text)
...
begin
  ...
  if p_findings is null or jsonb_typeof(p_findings) is distinct from 'array'
    or jsonb_array_length(p_findings) is null or jsonb_array_length(p_findings) not between 1 and 10000
  ...
  update public.plan_reading_jobs set status='needs_review',output_summary=p_summary,completed_at=now(),processing_error=null where id=p_job_id returning * into j;
...
create or replace function public.finish_owner_free_reading(p_job_id uuid,p_user_id uuid,p_summary jsonb,p_findings jsonb,p_error text)
returns jsonb language plpgsql security definer set search_path = public, private, pg_temp as $$
...
begin
  ...
  if not exists(select 1 from private.free_owner_workspaces where workspace_id=j.workspace_id) or j.requested_by<>p_user_id then
...
create or replace function public.finish_platform_admin_reading(p_job_id uuid,p_user_id uuid,p_summary jsonb,p_findings jsonb,p_error text)
returns jsonb language plpgsql security definer set search_path = public, private, pg_temp as $$
declare j public.plan_reading_jobs;
begin
  select * into j from public.plan_reading_jobs where id=p_job_id for update;
  if not found or j.status<>'processing' or j.requested_by<>p_user_id
    or j.input_summary->>'entitlement'<>'platform_admin_complimentary' then
    raise exception 'Reading is not authorized';
  end if;
  if not exists(select 1 from public.profiles where id=p_user_id and is_platform_admin=true) then raise exception 'Reading is not authorized'; end if;
  if not exists(select 1 from public.workspace_members where workspace_id=j.workspace_id and user_id=p_user_id and role in ('admin','estimator')) then
    raise exception 'Reading is not authorized';
  end if;
  if p_error is not null then
    update public.plan_reading_jobs set status='failed',processing_error=left(p_error,1000),completed_at=now() where id=p_job_id returning * into j;
