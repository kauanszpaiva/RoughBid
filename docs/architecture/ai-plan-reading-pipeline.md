...
5. **Extraction targets**
   ...
   - **Spaces the drawing draws but does not name.** A closet, bathroom, vestibule, corridor,
     shaft or stair is a space even when the sheet prints no label for it, so it is found from
     the drawn walls, the door swing and the fixtures and reported as its own `room` finding with
     a bbox. A printed label is not a requirement to report a location: when there is none, the
     finding quotes the drawn evidence and says so, with lower confidence. This was the single
     largest cause of a real plan "reading as text only" — the prompt used to ask for *labeled*
     rooms and to *cite a visible label*, which instructed the model to skip every unlabeled
     space on the sheet, and no amount of linework reinforcement could undo that instruction.
   - **Drawn openings and doors.** A doorway is not drawn as a labeled object: it is a wall that
     stops and starts again, with a leaf and a swing arc. Each drawn door, cased opening, passage
     and window is reported as a `symbol` finding with its own bbox even when untagged. A printed
     schedule count stays a quantity in its own finding; a drawn location never replaces a count
...
7. **Whole-set reading (one request per window)**
   ...
   - The sweep runs inline in the HTTP request, so the platform's function timeout is a hard ceiling on the whole reading. `AI_PLAN_SWEEP_BUDGET_MS` (240000, max 3600000) stops the sweep at a deadline and names the pages that were never read, instead of letting the platform kill the request mid-sweep with nothing saved. It must be configured below the deployed function's `maxDuration` (Vercel: `functions` → `maxDuration` in `vercel.json`, set to 300; Pro allows 300 s, Hobby caps at 60 s). `AI_PLAN_SWEEP_BUDGET_MS=0` means no deadline and is only safe on the durable worker, which has no HTTP timeout. The ceiling is one hour so a patient per-sheet reading can be configured; the default stays at four minutes so the inline path can never be pointed at a request the platform will kill.
   ...
   - The per-request output ceiling is 16000 tokens on the Gemini route (`MAX_OUTPUT_TOKENS`) and `AI_PLAN_OPENAI_MAX_OUTPUT_TOKENS` (16000, max 64000) on the OpenAI route: measured live, a window of 8 dense sheets returned `provider_output_truncated` at 8000 and its sheets produced no evidence at all. A truncated window is discarded whole, so this setting, not patience, is what decides whether a dense sheet is readable at all.
   - Controls: `AI_PLAN_GEMINI_BATCH_PAGES` / `AI_PLAN_GEMINI_MAX_BATCHES` / `AI_PLAN_GEMINI_TIMEOUT_MS` (120000, max 900000) / `AI_PLAN_GEMINI_SWEEP` for the Gemini route, `AI_PLAN_OPENAI_BATCH_PAGES` (4, max 50, forced to 1 while tiling) / `AI_PLAN_OPENAI_MAX_BATCHES` (25, max 400) / `AI_PLAN_OPENAI_TIMEOUT_MS` (120000, max 900000) / `AI_PLAN_OPENAI_MAX_OUTPUT_TOKENS` (16000, max 64000) / `AI_PLAN_OPENAI_SWEEP` for the OpenAI route, and `AI_PLAN_MAX_TOTAL_FINDINGS` (1000, max 3000) for the merged cap. The image-native providers never sweep: they read exactly the rendered pages they were given.
   - **Region (zoom) reading.** Waiting longer cannot resolve a door on a 1/8"=1'-0" overall plan: measured live, one dense sheet read as a single request returned **two** door openings, and the model itself reported `door and opening enumeration incomplete at overall-plan scale`. Angular resolution is the limit, not patience. `apps/api/src/ai-plan/page-tiles.ts` cuts a sheet into overlapping regions and each region is sent as its own metered request with the page's own CropBox moved (`cropSheetRegion`), so the provider displays that region zoomed — no renderer and no new dependency. Every box a region reports is measured from the top-left of **that region** and is restored to sheet coordinates deterministically (`mapNormalizedBoxFromRegion`); a box that cannot be placed is dropped rather than guessed, because the viewer would draw it. Two boundaries are deliberate: regions overlap, so a door on a seam is not cut in half and missed by both neighbours; and only a sheet whose **own measured drawing** is dense enough (`AI_PLAN_OPENAI_TILE_MIN_WALL_STROKES`, a measured stroke count and not a guess about the sheet's title) is cut, so a cover, a schedule or an elevation still costs one request. With tiling on, `AI_PLAN_OPENAI_MAX_BATCHES` becomes a provider-**request** budget and is spent as requests are actually made, because a real set mixes dense and sparse sheets; the pages the budget never reached, and any sheet it reached only partly, are named in `summary.limitations`.
   - **Patient reading.** A whole set read sheet by sheet, allowed to take 20-40 minutes, is a durable-worker configuration, not an inline one: no serverless request can be held open that long. `AI_PLAN_DURABLE_ENABLED=true` plus `REDIS_URL` and a running `scripts/worker.ts`, `AI_PLAN_SWEEP_BUDGET_MS=0`, `AI_PLAN_OPENAI_BATCH_PAGES=1` (or 2 for a large set), `AI_PLAN_OPENAI_TIMEOUT_MS=900000`, `AI_PLAN_OPENAI_MAX_OUTPUT_TOKENS=32000` and `AI_PLAN_OPENAI_MAX_BATCHES=60`. The worker builds the same provider set as the request handler (`apps/api/src/ai-plan/readers.ts`), so a provider named in `AI_PLAN_PROVIDER_ORDER` is available on both paths.
   - **The spend breaker gates a patient reading.** `reserve_provider_spend` reserves `provider_spend_policy.call_reservation_usd` per provider request, so the shipped default (2.50 of a 25.00 cap) admits about nine provider requests per day for the whole company — fewer than one per-sheet reading of a real set, and a reading that runs out mid-sweep saves the windows it already read and names the rest as unread. Lower the reservation once the real per-call cost is known: `update public.provider_spend_policy set call_reservation_usd = 0.25 where singleton;`
...
9. **Pricing and model behavior**
   - The provider order decides which reader runs, and the same order is used by the request handler and the durable worker (`apps/api/src/ai-plan/readers.ts`). OpenAI is first by default — the owner-selected reader for the drawing itself, PDF-native, so the local geometry pass and the printed-text transcript reinforce it instead of being the only evidence. Each reader disables SDK retries, so one paid attempt does not silently fan out into provider retries, and a reader that fails or returns nothing usable falls through to the next configured one.
...
- `AI_PLAN_SWEEP_BUDGET_MS`: wall-clock deadline for a whole-set sweep, kept below the deployed function's `maxDuration`. Missing means 240 s; the accepted maximum is 3600000 (one hour), and `0` means no deadline, which is only correct on the durable worker.
...
- `OPENAI_PLAN_READING_ENABLED` + `OPENAI_API_KEY` + `OPENAI_MODEL` (+ optional host-pinned `OPENAI_BASE_URL`): OpenAI as a paid plan reader. It attaches the plan PDF directly (`data:application/pdf;base64,...`), so it needs no server-side page renderer, and prefers rendered page images when they exist. Default model `gpt-5.4`, measured on a real 17-page set: 254 rooms (including unlabeled SHWR/TUB/TR/STRG/ELEV/STAIR) and drawn door swings, 8.2 min, 154k tokens at one sheet per request. `AI_PLAN_OPENAI_BATCH_PAGES` (4) sets how many sheets share one request — `1` is the most careful setting, and the reason a 17-page set needs the durable worker rather than an HTTP request — and `AI_PLAN_OPENAI_MAX_OUTPUT_TOKENS` (16000) is what keeps a dense window from being truncated and discarded whole.
- `AI_PLAN_PROVIDER_ORDER`: default `openai,gemini,claude,kimi,deepseek`. Unknown or unconfigured names are skipped; a reader is only built when its own gate passes, so the default changes nothing on a deployment whose OpenAI gate is closed.
...
- `AI_PLAN_OPENAI_BATCH_PAGES` / `AI_PLAN_OPENAI_MAX_BATCHES` / `AI_PLAN_OPENAI_SWEEP`: sweep window size, request cap and off switch for the OpenAI reader. With `AI_PLAN_OPENAI_TILE_GRID` above 1 the cap counts provider **requests**, not sheets.
- `AI_PLAN_OPENAI_TILE_GRID` (1 = off, max 3) / `AI_PLAN_OPENAI_TILE_OVERLAP` (0.12) / `AI_PLAN_OPENAI_TILE_MIN_WALL_STROKES` (1500) / `AI_PLAN_OPENAI_TILE_MAX_PAGES` (12): region reading of a dense sheet, so a door swing is resolved instead of merged by a whole-sheet view.
- `AI_PLAN_OPENAI_MAX_OUTPUT_TOKENS`: output ceiling for one OpenAI request (16000, max 64000). Too low and a dense window is discarded whole.
- `AI_PLAN_OPENAI_TIMEOUT_MS`: per-request ceiling for the OpenAI-compatible reader (default 120000, max 900000). Multi-page PDF requests are slower than single images.
- `AI_PLAN_MAX_TOTAL_FINDINGS`: ceiling on findings kept from a whole-set sweep (1000, max 3000). Measured: a real 17-page set reported 640 findings across its windows, so a lower cap silently discards evidence that was already read and paid for.
...
- `provider_spend_policy.call_reservation_usd`: the company breaker reserves this much per provider request and releases it once real telemetry is captured, so a reading is admitted only while `used + reservation <= spend_cap_usd`. The shipped default (2.50 of 25.00) allows about nine requests per 24 h — fewer than one careful reading of a real set now that a window can hold as few as one sheet, and the reason a long reading can end early with its unread pages named. A measured four-sheet reading on gpt-4o-mini costs roughly $0.0016, so lower the reservation to admit the readings you intend: `update public.provider_spend_policy set call_reservation_usd = 0.25 where singleton;`. `reserve_provider_spend` accepts only providers that migration 20260924134500 approves (gemini, deepseek, kimi, openai, claude); anything else fails closed before a request is made.
...
1. **OpenAI** — PDF-native reader selected by the owner for the drawing itself; window size, per-request timeout and output ceiling are all configurable, which is what makes a patient per-sheet reading possible.
2. **Gemini** — PDF-native reader with the same evidence contract; paid-service data is not used for product/model improvement under Google's published terms.
3. **Claude** — PDF-native reader with the same evidence contract, metered per call against the company spend policy.
