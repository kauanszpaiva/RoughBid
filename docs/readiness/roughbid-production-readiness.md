...
- Favicon and in-app mark use a RoughBid `R` plus construction-home signal.
...
- Connect the plan-reading AI pipeline and require estimator approval before any AI-suggested quantity affects a bid.
...
- Stripe: live and test accounts are connected; no product price or live charge should be created without commercial approval.
- Resend: no webhook is configured yet; transactional templates still need product-approved copy before sends.
- Tavily: useful for research and benchmarking, but not connected to the app runtime today.
