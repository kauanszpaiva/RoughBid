...
The existing Resend sending domain `mail.kspdominion.group` was verified on 2026-09-08; existing RoughBid sign-in and organization invitations had delivered statuses that day. No pilot invitations were sent during development. No Resend delivery webhook was configured in the initial audit. The release includes a signed webhook handler at `/api/webhooks/resend`; connect the endpoint and set `RESEND_WEBHOOK_SECRET` to persist delivery/bounce events in the owner dashboard. The handler authenticates the raw request body, checks a five-minute timestamp tolerance, and stores deduplicated provider event IDs.
